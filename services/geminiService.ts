...
import { HexagramData, TarotCard, BirthData } from "../types";
...
export const interpretHexagram = async (
  ...
  const methodText = hexagram.method === 'coin' ? '铜钱起卦 (六爻)' : '梅花易数 (时间起卦)';
  ...
  try {
    ...
    return response.text || "云深不知处，天机暂未明。请稍后再试。";
  } catch (error) {
    console.error("Gemini API Error:", error);
    return "连接神谕时发生错误，请检查网络后重试。";
  }
};
...
export const interpretGuanYin = async (
  ...
  try {
    ...
    return response.text || "佛光隐现，请稍后再试。";
  } catch (error) {
    console.error("Gemini API Error:", error);
    return "连接神谕时发生错误，请检查网络后重试。";
  }
...
export const interpretTarot = async (
  ...
  const cardDescriptions = cards.map(c => 
    `${c.position === 'past' ? '过去' : c.position === 'present' ? '现在' : '未来'}: 【${c.name} (${c.nameEn})】 - ${c.isReversed ? '逆位 (Reversed)' : '正位 (Upright)'}`
  ).join('\n');
  ...
  try {
    ...
    return response.text || "水晶球迷雾重重，请稍后再试。";
  } catch (error) {
    console.error("Gemini API Error:", error);
    return "连接宇宙能量时发生中断，请检查网络。";
  }
};
...
export const interpretBazi = async (
  ...
  const genderStr = birthData.gender === 'male' ? '男 (乾造)' : '女 (坤造)';
  const locationStr = birthData.location ? `出生地点: ${birthData.location}` : '';
  ...
  try {
    ...
    return response.text || "天干地支运转繁复，暂未算出结果，请稍后再试。";
  } catch (error) {
    console.error("Gemini API Error:", error);
    return "连接命运齿轮时发生错误，请检查网络。";
  }
};
...
export const interpretVedic = async (
  ...
  const genderStr = birthData.gender === 'male' ? '男' : '女';
  const locationStr = birthData.location || "未提供 (默认经纬度)";
  ...
  try {
    ...
    return response.text || "星海浩渺，云雾遮眼，请稍后再试。";
  } catch (error) {
    console.error("Gemini API Error:", error);
    return "连接宇宙能量场时发生中断，请检查网络。";
  }
};