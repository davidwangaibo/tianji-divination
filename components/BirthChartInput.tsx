...
import { BirthData, DivinationMethod } from '../types';
...
export const BirthChartInput: React.FC<BirthChartInputProps> = ({ onComplete, method = DivinationMethod.BAZI }) => {
  ...
  const handleSubmit = () => {
    if (date && time) {
      ...
      if (isVedic && !location.trim()) {
        alert("印度占星需要精确的出生地点来计算上升星座，请输入城市名称。");
        return;
      }
    ...
    <div className="flex flex-col items-center justify-center w-full max-w-md mx-auto p-8 bg-slate-800/50 rounded-xl border border-slate-700 shadow-xl backdrop-blur-sm animate-fade-in">
      ...
      <div className="mb-8 text-center">
        ...
        <h3 className={`text-xl font-cinzel font-bold mb-2 ${isVedic ? 'text-indigo-300' : 'text-amber-400'}`}>
          {isVedic ? '输入吠陀星盘信息' : '输入生辰八字'}
        </h3>
        <p className="text-slate-400 text-sm">
          {isVedic 
            ? '请输入出生信息，AI将根据恒星黄道（Sidereal Zodiac）为您排盘。' 
            : '请输入您的公历（阳历）出生信息，AI将为您排盘推演。'}
        </p>
      </div>
      ...
      <div className="w-full space-y-5">
        ...
        <div className="space-y-2">
          <label className="flex items-center gap-2 text-slate-300 text-sm font-bold uppercase tracking-wider">
            <Calendar size={14} className={isVedic ? "text-indigo-400" : "text-amber-500"} /> 出生日期 (公历)
          </label>
          <input 
            type="date" 
            value={date}
        ...
        <div className="space-y-2">
          <label className="flex items-center gap-2 text-slate-300 text-sm font-bold uppercase tracking-wider">
            <Clock size={14} className={isVedic ? "text-indigo-400" : "text-amber-500"} /> 出生时间
          </label>
          <input 
            type="time" 
            value={time}
            ...
            className={`w-full bg-slate-900/80 border rounded-lg p-3 text-slate-200 outline-none transition-all ${isVedic ? 'border-slate-600 focus:ring-2 focus:ring-indigo-500' : 'border-slate-600 focus:ring-2 focus:ring-amber-500'}`}
          />
          <p className="text-xs text-slate-500 italic px-1">准确的时间对于{isVedic ? '确定上升星座（Lagna）' : '确定时柱'}至关重要。</p>
        </div>

        {/* Location Input (New) */}
        <div className="space-y-2">
          <label className="flex items-center gap-2 text-slate-300 text-sm font-bold uppercase tracking-wider">
            <MapPin size={14} className={isVedic ? "text-indigo-400" : "text-amber-500"} /> 出生城市
          </label>
          <input 
            type="text" 
            value={location}
            onChange={(e) => setLocation(e.target.value)}
            placeholder="例如：中国北京市、上海市、London UK"
            className={`w-full bg-slate-900/80 border rounded-lg p-3 text-slate-200 outline-none transition-all ${isVedic ? 'border-slate-600 focus:ring-2 focus:ring-indigo-500' : 'border-slate-600 focus:ring-2 focus:ring-amber-500'}`}
          />
          {isVedic && <p className="text-xs text-indigo-400/70 italic px-1">* 印度占星必须提供地点以计算星盘。</p>}
        </div>

        {/* Gender Input */}
        <div className="space-y-2">
          <label className="flex items-center gap-2 text-slate-300 text-sm font-bold uppercase tracking-wider">
            <User size={14} className={isVedic ? "text-indigo-400" : "text-amber-500"} /> 性别
          </label>
