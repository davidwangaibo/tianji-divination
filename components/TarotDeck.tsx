...
export const TarotDeck: React.FC<TarotDeckProps> = ({ onComplete, lang }) => {
  ...
  const getPositionLabel = (pos: string) => {
    switch (pos) {
      case 'past': return lang === 'zh' ? '过去 (The Past)' : 'The Past';
      case 'present': return lang === 'zh' ? '现在 (The Present)' : 'The Present';
      case 'future': return lang === 'zh' ? '未来 (The Future)' : 'The Future';
      default: return '';
    }
  };
        ...
        {deckState === 'initial' ? (
          <div className="flex flex-col items-center animate-fade-in">
            ...
            <p className="mt-6 text-indigo-300 font-cinzel text-lg tracking-wider">
              {isShuffling ? (lang === 'zh' ? '洗牌中...' : 'Shuffling...') : (lang === 'zh' ? '点击牌堆开始洗牌' : 'Click deck to shuffle')}
            </p>
          </div>
        ) : (
          <div className="flex flex-col items-center w-full h-full justify-center">
            ...
            <p className="absolute bottom-4 text-indigo-300/50 text-xs font-mono">
              {drawnCards.length === 0 ? (lang === 'zh' ? '请抽取第一张牌：过去' : 'Draw 1st Card: Past') :
                drawnCards.length === 1 ? (lang === 'zh' ? '请抽取第二张牌：现在' : 'Draw 2nd Card: Present') :
                  drawnCards.length === 2 ? (lang === 'zh' ? '请抽取第三张牌：未来' : 'Draw 3rd Card: Future') : (lang === 'zh' ? '牌阵已成，正在解析...' : 'Spread complete. Analyzing...')}
            </p>
          </div>
        )}
      </div>
