...
import { YaoValue, Language } from '../types';
...
export const CoinToss: React.FC<CoinTossProps> = ({ onComplete, onReset, lang }) => {
  ...
  const finalizeToss = () => {
    ...
    if (tails === 1 && heads === 2) {
      resultStr = lang === 'zh' ? "一背二字" : "1 Tail 2 Heads";
      yaoStr = lang === 'zh' ? "少阴 (8)" : "Young Yin (8)";
    }
    else if (tails === 2 && heads === 1) {
      resultStr = lang === 'zh' ? "二背一字" : "2 Tails 1 Head";
      yaoStr = lang === 'zh' ? "少阳 (7)" : "Young Yang (7)";
    }
    else if (tails === 3) {
      resultStr = lang === 'zh' ? "三背 (重)" : "3 Tails (Moving)";
      yaoStr = lang === 'zh' ? "老阴 (6) 动" : "Old Yin (6) Moving";
    }
    else {
      resultStr = lang === 'zh' ? "三字 (交)" : "3 Heads (Moving)";
      yaoStr = lang === 'zh' ? "老阳 (9) 动" : "Old Yang (9) Moving";
    }
    ...
    <div className="grid md:grid-cols-2 gap-6 w-full max-w-2xl mx-auto">
      ...
      <div className="flex flex-col items-center justify-center p-6 bg-slate-800/50 rounded-xl border border-slate-700 shadow-xl backdrop-blur-sm">
        <h3 className="text-xl font-cinzel text-amber-400 mb-6 font-bold">
          {lines.length < 6 ? (lang === 'zh' ? `掷第 ${currentStep} 爻` : `Toss Line ${currentStep}`) : (lang === 'zh' ? '起卦完成' : 'Completed')}
        </h3>
        ...
        <div className="flex gap-4 mb-8 h-24 items-center">
          {coins.map((val, idx) => (
            <div
              key={idx}
              className={`w-16 h-16 rounded-full border-4 flex items-center justify-center text-xl font-bold shadow-lg transition-all duration-300 ${isTossing ? 'animate-spin scale-110' : 'scale-100'
                } ${val === 0 ? 'border-slate-600 opacity-30' : 'border-amber-500 bg-amber-500 text-amber-900'}`}
            >
              {val === 0 ? '?' : val === 3 ? (lang === 'zh' ? '字' : 'H') : (lang === 'zh' ? '背' : 'T')}
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-4 w-full">
          ...
          >
            <Disc size={20} />
            {lines.length >= 6 ? (lang === 'zh' ? '完成' : 'Done') : (lang === 'zh' ? '掷铜钱' : 'Toss Coin')}
          </button>
      ...
      <div className="bg-slate-900/50 rounded-xl border border-slate-800 p-4 h-[300px] overflow-y-auto custom-scrollbar">
        <div className="flex items-center gap-2 text-slate-400 mb-4 pb-2 border-b border-slate-800">
          <ScrollText size={16} />
          <span className="text-sm font-bold uppercase">{lang === 'zh' ? '起卦记录' : 'History'}</span>
        </div>
        {records.length === 0 ? (
          <p className="text-slate-600 text-center text-sm mt-10">{lang === 'zh' ? '暂无记录，请开始掷币。' : 'No records yet.'}</p>
        ) : (
          <div className="space-y-3">
            {records.map((r, i) => (
              <div key={i} className="flex items-center justify-between text-sm bg-slate-800/40 p-2 rounded animate-fade-in-right">
                <span className="text-slate-500">{lang === 'zh' ? `第 ${r.step} 爻` : `Line ${r.step}`}</span>
